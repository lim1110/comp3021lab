...
//		int max = findMax(0, array.length - 1);
		int[] max = new int[3];
		Thread t1 = new Thread(() -> {max[0] = findMax(0, 29);});
		Thread t2 = new Thread(() -> {max[1] = findMax(30, 59);});
		Thread t3 = new Thread(() -> {max[2] = findMax(60, 89);});
		t1.start();
		t2.start();
		t3.start();
		...
		System.out.println("the max value is " + Math.max(max[0], Math.max(max[1], max[2])));
	}
